	...
	<!-- Modified in livewire/livewire/features/supportfileuploads/withfileuploads.php -->	
	  ...
	  <ul wire:sortable="updateOrder" class="grid grid-cols-1 gap-4">	  	
			...
			@foreach($order as $key => $order_detail)
				...
				<li wire:sortable.item="{{$order_detail['value']}}" wire:key="{{$order_detail['value']}}" class="flex justify-center focus:text-violet-300">
					...
					<div class="max-h-12 max-w-full lg:max-w-[600px] flex flex-auto gap-2 p-3 text-base font-bold text-gray-900 rounded-lg bg-gray-50 hover:bg-gray-100 group hover:shadow dark:bg-gray-600 active:text-violet-">
						...
						<span class="inline-block w-[80%] text-ellipsis overflow-hidden select-none">
							{{$pdfs[$order_detail['value']]->getClientOriginalName()}}
						</span>

						<button wire:click="delPdf({{$order_detail['value']}},{{$key}})" class="flex w-7 sm:px-2 h-auto items-center justify-center px-2 py-0.5 text-xs font-medium text-gray-500 bg-gray-200 rounded-full border-solid border-2 hover:border-red-300 hover:text-red-300">X	</button>
